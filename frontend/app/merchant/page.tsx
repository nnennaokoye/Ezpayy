...
export default function MerchantPage() {
  ...
  const publicClient = useMemo(() => {
    return createPublicClient({
      ...
      transport: http(MANTLE_SEPOLIA.rpcUrls.default.http[0])
    })
  }, [])
    ...
    try {
      ...
      const onchain = [
        ...
        ...billLogs.map((log: any) => {
          const tAddr = (log.args.token as string).toLowerCase()
          const isNative = tAddr === TOKENS.MNT.toLowerCase()
          const meta = isNative ? { symbol: 'MNT', decimals: 18 } : (tokenMeta[tAddr] || { symbol: 'TOKEN', decimals: 18 })
          return {
            id: `${log.blockNumber.toString()}-${log.logIndex}`,
            bn: Number(log.blockNumber),
            li: Number(log.logIndex),
  ...
  const generatePayment = async () => {
    ...
    try {
      ...
      const encoded = encodeAbiParameters(
        ...
        [address, tokenAddress, BigInt(selectedChainId), EZPAY_CONTRACT_ADDRESS]
      )
      ...
      const link = `${window.location.origin}/pay?${amtParam}token=${encodeURIComponent(token)}&receiver=${address}&chainId=${selectedChainId}&contract=${EZPAY_CONTRACT_ADDRESS}&sig=${sig}`
      setPaymentLink(link)
      setQrGenerated(true)
  ...
  const shareLink = async () => {
    try {
      if (navigator.share) {
        await navigator.share({ title: 'Ezpay Payment', text: description || 'Pay with Ezpay', url: paymentLink })
      } else {
        copyLink()
      }
    } catch {}
  ...
  const downloadQR = async () => {
    try {
      ...
      a.href = url
      a.download = `ezpay-qr-${Date.now()}.png`
      a.click()
  ...
  const shareQR = async () => {
    try {
      if (!paymentLink) return
      const pngBlob = await generatePngBlobFromLink(paymentLink)
      const file = new File([pngBlob], `ezpay-qr-${Date.now()}.png`, { type: 'image/png' })
      const navAny: any = navigator
      if (navAny?.canShare && navAny.canShare({ files: [file] }) && navAny?.share) {
        await navAny.share({ files: [file], title: 'Ezpay QR Code', text: description || 'Scan to pay' })
      } else {
                    ...
                    {createdPayments.map((payment) => {
                      ...
                      <div key={payment.id} className="border rounded-lg p-4 hover:bg-muted/50 transition-colors">
                        ...
                        <div className="flex space-x-2 items-center">
                          ...
                          <Button onClick={() => navigator.share?.({ title:'Ezpay Payment', url: payment.link })}
              ...
              <Card>
                <CardHeader>
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <a className="text-xs text-muted-foreground" href={MANTLE_SEPOLIA.blockExplorers.default.url} target="_blank" rel="noreferrer">Explorer</a>
                  </div>
                      ...
                      <div key={tx.id} className="border rounded-lg p-4 hover:bg-muted/50 transition-colors">
                        <div className="flex justify-between items-start mb-3">
                          ...
                          <div className="text-right">
                            ...
                            <a href={`${MANTLE_SEPOLIA.blockExplorers.default.url}/tx/${tx.txHash}`} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline">View Tx</a>
                          </div>
                        </div>
