...
import { PlusCircle, QrCode, Copy, Zap, Wallet, Shield, Grid3X3, Eye, Trash2, ExternalLink, Calendar, DollarSign, Users, TrendingUp, Download, Share2 } from 'lucide-react';
...
export default function CreateBillPage() {
  ...
  const canCreate = receiver && amount && selectedToken && walletConnected && canProceed;
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <Button
                  ...
                  className="btn-animate tap-target"
                  size="lg"
                >
                ...
                <Button
                  onClick={() => {
                    // Simple download functionality - create a text file with payment URL
                    const element = document.createElement('a');
                    const file = new Blob([`Ezpay Payment Request\n\nAmount: ${amount} ${selectedTokenData?.symbol || 'ETH'}\nReceiver: ${receiver}\nPayment URL: ${paymentUrl}`], {type: 'text/plain'});
                    element.href = URL.createObjectURL(file);
                    element.download = `ezpay-payment-${billId}.txt`;
                ...
                <Button
                  onClick={() => {
                    if (navigator.share) {
                      navigator.share({
                        title: 'Ezpay Payment Request',
                        text: `Pay ${amount} ${selectedTokenData?.symbol || 'ETH'} securely with Ezpay`,
                        url: paymentUrl
                      });
                    } else {
                      copyPaymentUrl();
